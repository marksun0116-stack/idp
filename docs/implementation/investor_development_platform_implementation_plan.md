...
| Phase 10 — Advanced Technical Analysis | Integrate finance-data-service technical analysis into strategy research UI (indicators, recommendations, similar setups). | in_progress | Phase 9 | Each sprint can advance in parallel after finance-service API connectivity is verified | CONR-technical-analysis-api-001, CONR-finance-service-api-001, INV-strategy-validation-001 |
...
| RR-010 | Phase 10 | Review Phase 10 technical analysis integration requirements before frontend implementation starts. | done | US-018 | No | CONR-technical-analysis-api-001, CONR-finance-service-api-001 | Completed RR-010. Verified finance-data-service `/api/finance/analysis/{symbol}` endpoint operational. Confirmed response contains indicators (SMA20/50, RSI, MACD, Signal, Histogram) and recommendation (regime, confidence, winRate, similarSetups). Documented data transform and UI integration in `phase_10_rr_010_requirements_review.md`. Ready for US-019 implementation. |
| US-019 | Phase 10 | As an investor, I want to fetch and display technical indicators for strategy symbols so I can see current regime and signal alignment. | done | RR-010 | No blocking parallel work. | CONR-technical-analysis-api-001, FEAT-finance-technical-indicators-001 | Completed: Updated ExpandedAnalysisPanel with indicator toggle state (SMA20, SMA50, RSI, MACD). Enhanced TechnicalChartPanel to render indicator lines with color-coded buttons, secondary Y-axes for RSI/MACD, and dashed overlay lines. Integrated with existing IDP backend `/api/strategies/{id}/analysis/{symbol}?range=1y` endpoint which uses MarketDataService. Test by expanding a strategy symbol and toggling indicators in the chart tab. |
| US-020 | Phase 10 | As a researcher, I want a recommendation card in the expand panel showing regime, confidence, win rate, and similar historical setups. | done | US-019 | No blocking dependencies. | CONR-technical-analysis-api-001, INV-strategy-validation-001 | Completed: Enhanced IndicatorPanel in "Recommendation Details" tab to display recommendation card with regime label, strategy pattern, confidence badge (color-coded), reason text, trend signals, historical validation metrics (sample size, win rate, median return, status), and detailed similar setups table showing top 10 historical patterns with date, outcome (Win/Loss/Flat), and 20-day forward return. Table is scrollable for mobile and shows remaining count if >10 setups. |
| US-021 | Phase 10 | As an investor, I want a "Technical Outlook" widget on the strategy dashboard summarizing current regime and top 3 strategy confidence levels. | done | US-020 | No blocking dependencies. | FEAT-investor-development-platform-001, CONR-finance-service-api-001 | Completed: Added "Technical Outlook" panel to main dashboard showing top 3 tracked symbols from the selected strategy. Each symbol displays regime label (Bullish/Bearish/Neutral) with color-coding and confidence badge (High/Medium/Low). Shows "Select a strategy to view" when no strategy is selected. Panel has "View full" action link to navigate to portfolio view. Integrated symbolIndicators and selectedStrategy props to Dashboard. |
| US-022 | Phase 10 | As an investor, I want graceful fallback when technical analysis is unavailable so I understand why and can retry. | planned | US-021 | Can be addressed last after all display logic is in place. | CONR-technical-analysis-api-001, CON-strategy-analysis-scope-001 | Handles network timeouts, insufficient historical data, missing symbols, and finance-service unavailability; shows clear error messages and loading states; retries data on strategy refresh. |
...
- **Phase 10 is a consumer-only integration**: No backend work required; all technical analysis is in finance-data-service (FEAT-finance-technical-indicators-001, FEAT-finance-pattern-matching-001). US-019, US-020, US-021 can proceed in parallel once RR-010 confirms API connectivity and response shape. US-022 (fallback handling) depends on all display logic and should come last.
...
- 2026-06-09: **Restructured Phase 10** from backend implementation to frontend consumer integration. Finance-data-service already provides all technical analysis (FEAT-finance-technical-indicators-001, FEAT-finance-pattern-matching-001) via `/api/finance/analysis/{symbol}`. Phase 10 now focuses on UI integration: indicator display in charts, recommendation cards in expand panels, dashboard widgets, and graceful fallback handling. Changed from 5 backend sprints to 4 frontend sprints. Marked RR-010 as planning checkpoint before implementation stories start.
- 2026-06-09: **Completed RR-010 requirements review**. Verified finance-data-service endpoint operational and response contains indicators (SMA20/50, RSI, MACD, Signal, Histogram) and recommendation (regime, confidence, winRate). Documented data transform, UI integration points (ExpandedAnalysisPanel component), and error handling scenarios. Identified 232 valid indicator bars out of 251 for 1y range. Ready to start US-019 indicator display implementation. Clarifications pending from finance-data-service: EMA/Stochastic/OBV/MFI availability, similarSetups structure, edge case handling.
- 2026-06-10: **Completed US-019 indicator display**. Added indicator toggle buttons (SMA20, SMA50, RSI, MACD) to ExpandedAnalysisPanel with state management. Enhanced TechnicalChartPanel to parse indicator data from backend response and render overlay lines with color-coded styling (SMA20: blue, SMA50: orange, RSI: purple, MACD: pink). Implemented secondary Y-axes for RSI (0-100) and MACD to prevent scale conflicts with price. Integrated with existing IDP backend `/api/strategies/{id}/analysis/{symbol}` endpoint (uses MarketDataService). Ready for user testing in strategy expand panel. Next: US-020 recommendation card in expand panel.
- 2026-06-10: **Completed US-020 recommendation card**. Enhanced IndicatorPanel ("Recommendation Details" tab) to display full recommendation card with regime label (Bullish/Bearish/Neutral) with color-coding, strategy pattern, confidence badge (High/Medium/Low with green/yellow/red), reason text, trend signals (indicators with Buy/Sell signals and details), historical validation section (sample size, win rate, median return, validation status), and detailed scrollable table of top 10 similar historical setups with date, outcome (Win/Loss/Flat with color-coding), and 20-day forward return percentage. Also fixed 500 error on /api/decisions by adding @JsonIgnore to lazy-loaded InvestmentDecision collections. Ready for dashboard integration (US-021).
- 2026-06-10: **Completed US-021 dashboard widget**. Added "Technical Outlook" panel to main dashboard showing top 3 tracked symbols from the selected strategy with regime label (Bullish/Bearish/Neutral, color-coded) and confidence badge (High/Medium/Low with color). Panel shows "Select a strategy to view" message when no strategy selected. Added "View full" action to navigate to portfolio view. Passed symbolIndicators, selectedStrategy, and strategyQuotes props to Dashboard component. Now only US-022 (fallback handling) remains for Phase 10 completion.
...
| 4.0 | 2026-06-09 | active | 3.9 | **Restructured Phase 10**: Discovered finance-data-service already implements all technical analysis (FEAT-finance-technical-indicators-001, FEAT-finance-pattern-matching-001). Refactored Phase 10 from 5 backend sprints (indicator implementation, pattern matching, recommendation engine) to 4 frontend sprints (indicator chart display, expand panel recommendations, dashboard widget, fallback handling). Updated CONR-technical-analysis-api-001 to consumer contract. Added RR-010 through US-022 user stories for Phase 10 focused on integration with finance-data-service `/api/finance/analysis/{symbol}` endpoint. |
| 4.1 | 2026-06-09 | active | 4.0 | **Completed Phase 10 RR-010**. Verified finance-data-service endpoint operational; confirmed response contains 251 indicator bars with SMA20/50, RSI, MACD, Signal, Histogram. Identified ExpandedAnalysisPanel component as integration point. Documented data transform and error handling scenarios. Marked US-019 as in_progress. |
| 4.2 | 2026-06-10 | active | 4.1 | **Completed US-019 indicator display**. Added indicator toggle buttons to ExpandedAnalysisPanel (SMA20, SMA50, RSI, MACD with color-coding). Enhanced TechnicalChartPanel to render indicator overlays with secondary Y-axes for RSI/MACD. Integrated with IDP backend `/api/strategies/{id}/analysis/{symbol}` endpoint. Ready for user testing. |
| 4.3 | 2026-06-10 | active | 4.2 | **Completed US-020 recommendation card** and **fixed /api/decisions 500 error**. Enhanced IndicatorPanel with recommendation card showing regime/confidence/signals and detailed similar setups table (top 10 historical patterns). Fixed Hibernate lazy initialization by adding @JsonIgnore to InvestmentDecision alert/editHistory collections. |
| 4.4 | 2026-06-10 | active | 4.3 | **Completed US-021 dashboard widget**. Added "Technical Outlook" panel to main dashboard showing top 3 tracked symbols from selected strategy with regime label and confidence badge. Integrated technical analysis data with dashboard. |
