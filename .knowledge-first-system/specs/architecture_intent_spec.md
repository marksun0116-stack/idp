...
| File | Purpose |
| --- | --- |
| `.knowledge-first-system/specs/architecture_intent_spec.md` | This file — **how** to write architecture intent. |
...
**This knowledge-first-system package** may ship **without** a `docs/` tree. When you add architecture intent in-repo, **create `docs/architecture/`** first (**`.knowledge-first-system/README.md`**). Filenames are not fixed; keep them **stable and searchable** (avoid frequent renames so trace links from reviews stay valid).
...
Every architecture intent document under **`docs/architecture/`** must end with **`## Change log`** or **`## Version history`** as specified in **§3** above. Specifications under **`.knowledge-first-system/specs/`** use the same terminal pattern for their own revision history.
